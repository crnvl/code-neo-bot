...
public class kick implements runinterface {
    @Override
    ...
    private void send(String[] args, MessageReceivedEvent event) {
        if (event.getGuild().getMember(event.getMessage().getAuthor()).hasPermission(Permission.KICK_MEMBERS)) {
            if (args.length != 1) {
                ...
                if (isMention(args[0])) {
                    ...
                    event.getTextChannel().sendMessage(
                            new EmbedBuilder().setColor(Color.BLACK)
                                    .setTitle("Kick")
                                    .setDescription("**" + event.getMessage().getMentionedMembers().get(0).getAsMention() + "** _has been kicked!_").build()).queue();
                } else if (event.getGuild().getMemberById(args[0]) != null) {
                    event.getGuild().getController().kick(event.getGuild().getMemberById(args[0])).queue();
                    event.getTextChannel().sendMessage(
                            new EmbedBuilder().setColor(Color.BLACK)
                                    .setTitle("Kick")
                                    .setDescription("**" + event.getGuild().getMemberById(args[0]).getAsMention() + "** _has been kicked!_").build()).queue();
                } else {
